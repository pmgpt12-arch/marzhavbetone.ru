...
def build() -> tuple[list[dict[str, str]], dict[str, int]]:
    ...
    for key, members in sorted(groups.items()):
        ...
        for member in members:
            ...
            elif existing and own == existing:
                # Запрос перенацелен на ту же страницу, что и носитель, —
                # интент уже обслужен, никакого второго материала нет.
                decision = "no-page"
                target = existing
                note = (
                    f"перенацелен на страницу носителя "
                    f"«{carrier.get('Запрос','')}»"
                )
