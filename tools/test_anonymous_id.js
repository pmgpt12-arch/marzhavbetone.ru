...
проверка('anonymous_id_is_not_pii', () => {
  /* Проверяется теми же образцами, какими поток событий вырезает ПД, а не
     «нет ли семи цифр подряд»: в UUID шестнадцатеричные цифры идут подряд
     сплошь и рядом, и такая проверка падала бы примерно в двух случаях из
     трёх — на собственной случайности, а не на дефекте. Найдено прогоном.

     Сто значений, а не одно: свойство «не содержит ПД» обязано держаться
     на любом, и один удачный образец его не показывает. */
  const ПОЧТА = /[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,}/;
  const ТЕЛЕФОН = /(?:\+7|\b8)[\s(-]*\d{3}[\s)-]*\d{3}[\s-]*\d{2}[\s-]*\d{2}\b/;
  const виденные = new Set();
  for (let i = 0; i < 100; i++) {
    const id = песочница({ jar: new Map() }).ctx.window.mvbAnonymousId();
    истинно(UUID4.test(id), 'не UUIDv4: ' + id);
    истинно(!ПОЧТА.test(id), 'похоже на адрес почты: ' + id);
    истинно(!ТЕЛЕФОН.test(id), 'похоже на номер телефона: ' + id);
    виденные.add(id);
  }
  // Одинаковый на всех идентификатор — не идентификатор, а константа.
  равно(виденные.size, 100, 'значения повторяются у разных посетителей');
});
