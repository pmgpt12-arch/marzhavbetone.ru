...
Семантика лежит в `content/strategy/semantic-core.csv`, колонка «Целевой
URL». Ни один инструмент репозитория её с реальностью не сверял, и
расхождение накопилось молча: замер 13.08.2026 показал, что из целевых URL
семантики в `sitemap.xml` не попал ни один, а сотни запросов метили в раздел
`/baza-znanii/`, которого на сайте нет. Обходом живого сайта такое не
находится — семантика лежит в `content/`, который не деплоится.

Замер 14.08.2026 после разбора: восемь материалов, лежавших по адресу
отличному от планового, приведены к фактическому; заготовки из счёта
исключены. Осталось 277 запросов, метящих в ненаписанные страницы, — это
...
# находка. 404.html — экран ошибки: она noindex сама по себе и отдаётся
# на любом мусорном адресе, в карте ей места нет вдвойне (MB008).
NOT_FOR_SEARCH = {
    "/success.html",
    "/fail.html",
    "/test-pokupka.html",
...
def semantic_rows() -> list[dict[str, str]]:
    """Строки семантики без заготовок.

    Заготовки — шаблонные записи «основа — хвост», не поисковые запросы
    (см. `tools/keyword_map.py`). Их триста, и в счёте «сколько запросов
    метит в несуществующий раздел» они раздували число втрое: решение
    владельцу подавалось крупнее, чем оно есть.
    """
    if not SEMANTIC.exists():
        return []
    with SEMANTIC.open(encoding="utf-8") as fh:
        return [row for row in csv.DictReader(fh)
                if row.get("Статус", "").strip() != "Заготовка"]
