...
# Перечень официальных публикаторов. Строка `source_kind: official` сама по
# себе ничего не доказывает: её пишет тот же человек, что и текст. Поэтому
# происхождение сверяется локально — по объявленному публикатору и адресу,
# без единого сетевого запроса. Проверяется НЕ доступность страницы (её
# закрытость ничего не говорит о подлинности акта), а то, что публикатор
# назван из закрытого перечня и адрес принадлежит его домену.
ПУБЛИКАТОРЫ = {
    ...
    "sudact.ru": {"название": "Судебные и нормативные акты РФ",
                  "домены": ("sudact.ru",)},
...
def check_origin(source: dict) -> list[str]:
    """Происхождение источника: публикатор из перечня и адрес его домена.

    Сети здесь нет намеренно. Доступность страницы подлинности не доказывает
    (и из части сетей официальные публикаторы попросту закрыты), а вот
    несовпадение объявленного публикатора с доменом адреса — проверяемый
    признак, и он ловится локально.
    """
    беды: list[str] = []
