...
Правила, каждое роняет прогон:
 ...
 8а. сверка покрыла не все применимые нормы документа;
...
def check() -> list[str]:
    ...
    for rel, doc in declared.items():
        ...
        # 11. Результат обязан покрывать ВСЕ применимые нормы документа.
        # Причина замером 16.08.2026: реестр расширился с 17 норм до 31, а
        # шесть результатов остались на прежнем наборе — и гейт этого не
        # заметил бы. Сверка по половине применимых норм это не сверка
        # документа, а сверка её части, и разница не видна ничем, кроме
        # такого пересчёта.
        применимо = set(doc.get("norms", []))
        сверено = {c["norm_id"] for c in result.get("norms_checked", [])}
        пропущено = sorted(применимо - сверено)
        if пропущено:
            defects.append(f"{short}: сверка не покрывает применимые нормы — "
                           + ", ".join(пропущено[:4])
                           + (f" и ещё {len(пропущено) - 4}"
                              if len(пропущено) > 4 else ""))
