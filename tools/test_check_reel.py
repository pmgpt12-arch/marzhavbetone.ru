...
ФИНАЛЬНЫЙ_ЭКРАН = "Как не кредитовать\nзаказчика бесплатно?\n\nСсылка в профиле"


def порча_убрать_призыв_с_экрана(text: str) -> str:
    return text.replace(ФИНАЛЬНЫЙ_ЭКРАН, "Вот так это и работает")


def порча_общий_призыв(text: str) -> str:
    return text.replace(ФИНАЛЬНЫЙ_ЭКРАН, "Узнайте больше\nСсылка в профиле")


def порча_подписка_вместо_действия(text: str) -> str:
    return text.replace(ФИНАЛЬНЫЙ_ЭКРАН, "Подпишитесь\nСсылка в профиле")
        ...
        try:
            for порча, ожидание in СЛУЧАИ:
                ...
                if испорченный == эталон:
                    # Порча искала точную строку, а эталон с тех пор
                    # переписан — .replace() молча ничего не нашёл, копия
                    # равна оригиналу, и случай проходит не потому что
                    # проверка ловит порчу, а потому что портить нечего.
                    провалов += 1
                    print(f"ПОРЧА НЕ СРАБОТАЛА  {порча.__name__}: "
                          f"текст не изменился — эталон переписан, "
                          f"искомая строка в нём больше не встречается")
                    continue
