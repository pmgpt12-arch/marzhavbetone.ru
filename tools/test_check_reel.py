...
# Призыв на экране эталона. Вынесен в одну строку намеренно: пока каждая
# порча несла свою копию этого текста, правка сценария расходилась с ними
# по одной, и порча превращалась в пустую замену.
ПРИЗЫВ_НА_ЭКРАНЕ = "Как не кредитовать\nзаказчика бесплатно?\n\nСсылка в профиле"


def порча_убрать_призыв_с_экрана(text: str) -> str:
    return text.replace(ПРИЗЫВ_НА_ЭКРАНЕ, "Вот так это и работает")


def порча_общий_призыв(text: str) -> str:
    return text.replace(ПРИЗЫВ_НА_ЭКРАНЕ, "Узнайте больше\n\nСсылка в профиле")


def порча_подписка_вместо_действия(text: str) -> str:
    return text.replace(ПРИЗЫВ_НА_ЭКРАНЕ, "Подпишитесь\n\nСсылка в профиле")
...
def порча_нет_стандарта(text: str) -> str:
    # Два пробела на конце — это разрыв строки разметки, и без них замена
    # не находит ничего
    ...
    with tempfile.TemporaryDirectory() as каталог:
        ...
        try:
            for порча, ожидание in СЛУЧАИ:
                ...
                # Порча, ничего не изменившая, — самый тихий вид поломки:
                # гейт честно молчит на исправном файле, а прогон читается
                # как дыра в гейте. Замер 23.08.2026: пять случаев из восьми
                # не меняли ни байта, потому что текст эталона правился, а
                # строки здесь остались прежними. Отличается это только так.
                if испорченный == эталон:
                    провалов += 1
                    print(f"ПОРЧА ПУСТА {порча.__name__}: текст эталона "
                          f"не изменился — строка порчи устарела")
                    continue
