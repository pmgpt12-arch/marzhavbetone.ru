...
# Призыв эталона одной строкой: он стоит в трёх порчах, и разъехавшись
# по ним, они молча перестали бы что-либо портить. Текст берётся из
# сценария дословно, вместе с пустой строкой между вопросом и адресом.
...
def порча_нет_стандарта(text: str) -> str:
    # Два пробела в конце — разрыв строки Markdown, часть значения. Без них
    # замена не срабатывала вовсе.
    ...
    with tempfile.TemporaryDirectory() as каталог:
        ...
        try:
            for порча, ожидание in СЛУЧАИ:
                ...
                # Сторож. `str.replace` на несовпавшем якоре молча возвращает
                # тот же текст: портить становится нечего, гейт справедливо
                # молчит, а прогон читает это как «проверка не ловит» и винит
                # исправный инструмент. Так и вышло, когда сценарий-эталон
                # переписали: пять случаев из восьми обвиняли check_reel.py.
                if испорченный == эталон:
                    провалов += 1
                    print(f"ПОРЧА НЕ ПРИМЕНИЛАСЬ  {порча.__name__}: "
                          "якорь разошёлся с эталоном, случай ничего не проверил")
                    continue
