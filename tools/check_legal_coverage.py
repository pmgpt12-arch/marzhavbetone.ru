...
Ссылка бывает двух родов, и различать их обязательно. **Нормативная
опора** — норма, на которой стоит юридический вывод документа; она в
реестре, и по ней нужно официальное доказательство. **Атрибутивная
ссылка** — акт назван как происхождение формы или как исторический факт,
вывод на нём не держится. Вторая объявляется в `attributive_references`
документа и обязательно с причиной: без причины это способ спрятать
непокрытую опору, и проверка на такое краснеет.
...
# Кодекс называют и аббревиатурой, и полным именем. Замер 16.08.2026:
# документ 12 пишет «В силу ст. 309, 310 Гражданского кодекса РФ», и
# проверка, знавшая только «ГК», дала ложный ноль непокрытых ссылок —
# ровно тот исход, ради предотвращения которого она заведена. Нашёл это
# не код, а сверщик, и это цена узкого распознавания.
#
# Порядок в перечне значим: «Гражданский процессуальный кодекс» обязан
# разбираться раньше «Гражданского кодекса», иначе ГПК прочитается как ГК.
_ПОЛНЫЕ = [
    (r"Арбитражн\w*\s+процессуальн\w*\s+кодекс\w*", "АПК"),
    (r"Гражданск\w*\s+процессуальн\w*\s+кодекс\w*", "ГПК"),
    (r"Налогов\w*\s+кодекс\w*", "НК"),
    (r"Гражданск\w*\s+кодекс\w*", "ГК"),
]
_АКТ = "|".join([r"\b(?:ГК|АПК|НК|ГПК)\b"] + [p for p, _ in _ПОЛНЫЕ])
...
def _ключ_акта(текст: str) -> str:
    """Название акта из текста → ключ. Полное имя и аббревиатура равны."""
    сжато = текст.strip()
    if сжато in КОДЕКСЫ:
        return КОДЕКСЫ[сжато]
    for образец, ключ in _ПОЛНЫЕ:
        if re.fullmatch(образец, сжато, re.I):
            return ключ
    return ""
...
def ссылки(body: str) -> set[str]:
    ...
    out: set[str] = set()
    for номера, акт in _СТАТЬИ.findall(body):
        ключ = _ключ_акта(акт)
        if not ключ:
            continue
        for единица in _НОМЕР.findall(номера):
            out.add(f"{ключ}:{единица}")
...
def main() -> int:
    ...
    for doc in documents:
        ...
        # Атрибутивная ссылка: акт назван, но юридический вывод на нём не
        # стоит. Держать такую норму в опоре значит требовать доказательства
        # там, где вывода нет; умолчать — потерять ссылку. Поэтому она
        # объявляется отдельно и обязательно с причиной: без причины это
        # просто способ спрятать непокрытую опору, и проверка краснеет.
        атрибуты = doc.get("attributive_references") or []
        for запись in атрибуты:
            ref = str(запись.get("ref", "")).strip()
            если_нет_причины = not str(запись.get("reason", "")).strip()
            if not ref:
                непокрыто.append(f"{path.name}: атрибутивная ссылка без ref")
                continue
            if если_нет_причины:
                непокрыто.append(
                    f"{path.name}: {ref} объявлена атрибутивной без причины")
                continue
            if ref not in найдено:
                непокрыто.append(
                    f"{path.name}: {ref} объявлена атрибутивной, "
                    "но в тексте не найдена")
                continue
            покрыто.add(ref)
    ...
    атрибутивных = sum(len(d.get("attributive_references") or [])
                       for d in documents)
    if атрибутивных:
        print(f"\nАтрибутивных ссылок объявлено: {атрибутивных} — акт назван, "
              "юридический вывод на нём не стоит.")
        for doc in documents:
            for запись in doc.get("attributive_references") or []:
                имя = doc["path"].split("/")[-1]
                print(f"  · {имя}: {запись['ref']}")
    if лишнее:
