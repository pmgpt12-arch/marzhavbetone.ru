...
PPVS = {"act": "Постановление Пленума ВС РФ от 24.03.2016 № 7",
        "article": "пп. 42, 48, 84",
        "document_match": ["пленума верховного суда", "№ 7"]}

ЗАГОЛОВОК = ("ПОСТАНОВЛЕНИЕ ПЛЕНУМА ВЕРХОВНОГО СУДА РОССИЙСКОЙ ФЕДЕРАЦИИ "
...
def test_настоящий_pdf_постановления_если_он_приехал():
    """Настоящий ответ Верховного Суда — главное доказательство разбора.

    Ищется подписью `%PDF-`, а не расширением: захват прошлого прогона
    положил файл как `.html`, потому что PDF-путь тогда ещё не работал.
    Подпись — свойство содержимого, имя файла — нет.
    """
    pages = FIXTURES / "pages"
    pdfs = [p for p in sorted(pages.glob("*"))
            if p.is_file() and is_pdf(p.read_bytes()[:8])] if pages.exists() else []
    if not pdfs:
