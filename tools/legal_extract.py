...
def looks_truncated(html: str) -> bool:
    """Ответ оборван на полуслове, хотя код 200.

    Замер 16.08.2026: оболочка ГК части первой пришла на 4 561 байт вместо
    56 248 — заголовок верный, `</html>` отсутствует, разметка обрывается
    посреди тега `<select name="doc_editions"`. Фрейма с текстом в обрезке
    уже нет, и инструмент честно докладывал «пути к тексту не нашлось» —
    но причина не в отсутствии пути, а в том, что ответ недокачан.
    Разница важна: первое чинится разбором, второе — повтором запроса.
    """
    tail = html[-4000:].lower()
    return "</html>" not in tail and "</body>" not in tail


def identifies_act(html: str, body: str, terms: list[str]) -> bool:
    """Тот ли документ вообще пришёл.

    Проверка появилась не из осторожности, а по находке: адрес АПК из
    реестра (`nd=102078170`) отдаёт **Распоряжение Правительства от
    30.08.2002 № 1214-р**. Без этой проверки совпадения номера хватило бы,
    чтобы «Статья 4» чужого акта легла в доказательство под именем АПК —
    то есть гейт получил бы зелень на подложном тексте. Хуже отказа.

    Сверяются заголовок и начало документа: реквизиты стоят там у всех
    шести сохранённых ответов.
    """
    if not terms:
        return True
    where = f"{page_title(html)} {body[:3000]}".lower()
    return all(t.lower() in where for t in terms)
...
def strip_html(html: str) -> str:
    html = re.sub(r"<(script|style|noscript)\b.*?</\1>", " ", html,
                  flags=re.S | re.I)
    # Номер статьи с дополнительным индексом публикатор печатает надстрочным
    # тегом: «Статья 333<span class="W9">21</span>.» — это статья 333.21.
    # После снятия тегов тег превращался в пробел, получалось «Статья 333 21»,
    # и «Статья 333.21» не находилась ни разу — замер 16.08.2026 по
    # семимегабайтному печатному виду НК: вхождений ноль при 261 найденной
    # статье. Ровно на этом стояли оба отказа по НК.
    html = re.sub(r"(\d)\s*<(?:sup|span)\b[^>]*>\s*(\d{1,3})\s*</(?:sup|span)>",
