...
GROUPS = [
    ...
    ("zakrytie-i-oplata", "Закрытие и оплата",
     ...
     ["p3", "p1", "t2", "t1", "t3"]),
    # t5 стоит здесь, а не рядом со своим ядром p8, и это решение, а не
    # недосмотр. Группы раскладываются по моменту работы, а не по товарной
    # паре: подпись группы «До подписания» говорит «пока условия ещё можно
    # менять», а момент этой ступени — требование о возврате аванса по уже
    # прекращённому договору. «Спор о сумме» — ровно она.
