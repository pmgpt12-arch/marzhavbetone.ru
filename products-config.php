...
function mvb_products(): array
{
    return [
        ...
        // Входной товар боли neoplata. Между бесплатным чек-листом и комплектом
        // за 2 490 ₽ не было ничего: холодный посетитель должен был сразу
        // отдать полную цену. Здесь он покупает то, за чем пришёл по статье, —
        // досудебную часть целиком. Искового заявления тут нет намеренно: оно
        // остаётся в p1 (решение владельца 31.07.2026). С 20.09.2026 (MB005)
        // t1 и p1 — самостоятельные товары под разный объём задачи: полный
        // маршрут с иском покупается напрямую, зачёта стоимости ступени не
        // существует.
        't1' => [
        ...
        // оплаты остаются в p4, и это граница объёма между двумя товарами —
        // покупка t2 коммерческих условий p4 не меняет (MB005). Формы
        ...
        // Ядра узла стоят 2 990 ₽ (p10) и 9 900 ₽ (p12) против 990 ₽
        // ступени; разница — цена разных объёмов задачи, а не доплата при
        // переходе, зачёта стоимости ступени не существует (MB005).
