...
if (!is_file($zipPath)) {
    ...
    //
    // Пересобирается ИМЕННО то издание, что записано в заказе, а не текущее
    // издание каталога. Иначе старый оплаченный заказ после штатной чистки
    // кэша получил бы новый состав под своим прежним именем.
    $zipPath = mvb_build_product_zip($sku, $decision['zip']) ?? '';
}
